...
# credit to https://github.com/gluckzhang/prometheus2csv
...
def handle_args(argv):
    ...
    if OUTPUTFILE == "":
        ...
        logging.warning(
            "You didn't specify output file's name, will use default name %s",
            OUTPUTFILE,
        )
    if RESOLUTION == "":
        RESOLUTION = "10s"
        logging.warning(
            "You didn't specify query resolution step width, will use default value %s",
            RESOLUTION,
        )
    ...
    if LABEL == "":
        LABEL = "normal"
        logging.warning(
            "You didn't specify the label, will use default value %s",
            LABEL,
        )
...
def print_help_info():
    ...
    print("---")
    print(
        "Additional options: --start=<start_timestamp_or_rfc3339> --end=<end_timestamp_or_rfc3339> --period=<get_for_most_recent_period(int miniutes)> --label=<label_name>"
    )
...
def query_metric_names():
    response = requests.get(
        PROMETHEUS_URL + QUERY_API,
        params={"query": 'sum by(__name__)({{name="{0}"}})'.format(CONTAINER)},
    )
...
def query_metric_values(metricnames):
    ...
    response = requests.get(
        PROMETHEUS_URL + RANGE_QUERY_API,
        params={
            "query": '{0}{{name="{1}"}}'.format(metric, CONTAINER),
            "start": start_time,
            "end": end_time,
            "step": RESOLUTION,
        },
...
def write2csv(filename, metricnames, dataset):
    with open(filename, "w") as file:
        ...
        writer.writerow(["timestamp"] + metricnames + ["label"])
        for timestamp in sorted(dataset.keys(), reverse=True):
            writer.writerow([timestamp] + dataset[timestamp] + [LABEL])
        # for line in dataset:
